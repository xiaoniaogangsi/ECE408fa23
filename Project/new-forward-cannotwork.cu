...
__global__ void conv_forward_kernel(float *output, const half *input, const half *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    /*Optimization 5: Fixed point (FP16) arithmetic*/

    /*Optimization 6: Using Tensor Cores to speed up matrix multiplication*/

    __shared__ half subTile_input[TILE_WIDTH * TILE_WIDTH];
    __shared__ half subTile_mask[TILE_WIDTH * TILE_WIDTH];
    __shared__ float subTile_output[TILE_WIDTH * TILE_WIDTH];
    ...
    //The leading dimensions of the three matrices.
    //Since we want mask(M*K) * input_unrolled(K*N) = output(M*N), and the matrices are in row-major form,
    //so the leading dimensions are the row dimensions (the latter ones).
    int ld_mask = WMMA_K;
    int ld_input = WMMA_N;
    int ld_output = WMMA_N;

    //Declare the fragments
    wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major> mask_frag;
    wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major> input_frag;
    wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, float> output_frag;

    //Initialize the output_frag to all zeros
    ...
    #define tile_in_2d(i1, i0) subTile_input[(i1) * (TILE_WIDTH) + (i0)]
    #define tile_mask_2d(i1, i0) subTile_mask[(i1) * (TILE_WIDTH) + (i0)]
    #define tile_out_2d(i1, i0) subTile_output[(i1) * (TILE_WIDTH) + (i0)]
    //tile_in_2d(ty, tx)
    //tile_mask_2d(ty, tx)
    //tile_out_2d(ty, tx)
    ...
    //Now I have 8 warps in a block, 2 rows combined into a warp.
    int WarpM = Row / 2;
    int WarpN = Col / 16;
    ...
    //load subtiles of A and B from the global memory to the shared memory of this block
        if (Row < num_mask_rows && i*TILE_WIDTH+tx < num_mask_columns){ //Boundary condition
            tile_mask_2d(ty, tx) = Mask_c[Row * num_mask_columns + i * TILE_WIDTH + tx];
        }else{
            tile_mask_2d(ty, tx) = __float2half(0.0f);
        }
        ...
        if (i*TILE_WIDTH+ty < num_input_rows && Col < num_input_columns) { //Boundary condition condition
            ...
            if ((h_out*S+p < H) && (w_out*S+q < W)){
                tile_in_2d(ty, tx) = in_4d(b, c, h_out*S + p, w_out*S + q);
            }else{
                tile_in_2d(ty, tx) = __float2half(0.0f);
            }
        ...
        int mask_Row_base = WarpM * WMMA_M;
        int mask_Col_base = i * TILE_WIDTH;
        int input_Row_base = i * TILE_WIDTH;
        int input_Col_base = WarpN * WMMA_N;

        wmma::load_matrix_sync(mask_frag, subTile_mask + ty * ld_mask + tx, ld_mask);   //ld_mask = TILE_WIDTH = 16
        wmma::load_matrix_sync(input_frag, subTile_input + ty * ld_input + tx, ld_input);   //ld_input = TILE_WIDTH = 16
        ...
        //         output_val += tile_mask_2d(ty, k) * tile_in_2d(k, tx);
        //         // output_val += Mask_c[Row * num_mask_columns + i * TILE_WIDTH + k] * tile_in_2d(k, tx);
        //     }
        // }
        wmma::mma_sync(output_frag, mask_frag, input_frag, output_frag); //Compute the matrix multiplication
    ...
    wmma::store_matrix_sync(subTile_output + ty * ld_output + tx, output_frag, ld_output, wmma::mem_row_major);
    if (Row < num_output_rows && Col < num_output_columns){ //Boundary condition
        // output[b * (num_output_rows * num_output_columns) + Row * num_output_columns + Col] = output_val;  //Store the result element back
        output[b * (num_output_rows * num_output_columns) + Row * num_output_columns + Col] = tile_out_2d(ty, tx);
    }
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    // cudaMemcpyToSymbol(Mask_c, host_mask, M * C * K * K * sizeof(half));
}
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    // Set the kernel dimensions and call the matrix multiplication kernel
    ...
    // conv_forward_kernel<<<dimGrid, dimBlock>>>(device_output_fp16, device_input_fp16, device_mask_fp16, B, M, C, H, W, K, S);
    conv_forward_kernel<<<dimGrid, dimBlock>>>(device_output, device_input_fp16, device_mask_fp16, B, M, C, H, W, K, S);
    cudaDeviceSynchronize();
