...
__global__ void convertFP32toFP16_padded(half *out, const float *in, int width, int height, const int M, const int C, const int K)
{
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    int m = idx / width;
    int idx_one_FM = idx % width;
    if (idx < width * height) {
        if ((m < M) && (idx_one_FM < C * K * K)) {
            out[idx] = __float2half(in[m*(C*K*K) + idx_one_FM]);
        } else {
            out[idx] = __float2half(0.0f); //padding
        }
    }
}

//Convert half (FP16) to float (FP32)
__global__ void convertFP16toFP32_unpadded(float *out, half *in, int width, int height, const int B, const int M, const int H_out, const int W_out)
{
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    int b = idx / B;
    int idx_one_image = idx % B;
    int m = idx_one_image / width;
    int idx_one_FM = idx_one_image % width;
    
    if (idx < B * width * height) {
        if ((m < M) && (idx_one_FM < H_out * W_out)) {
            out[(b) * (M * H_out * W_out) + (m) * (H_out * W_out) + (idx_one_FM)] = __half2float(in[idx]);
        }    
    }
}
...
__global__ void conv_forward_kernel(half *output, const half *input, const half *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    //When converting the mask, we should also do a padding, to make the mask width and height both a multiple of 16.
    int mask_height_padded = (M + 15)/16 * 16;
    int mask_width_padded = (C * K * K + 15)/16 * 16;
    //When unrolling the input, we should also do a padding, to make the input width and height both a multiple of 16.
    int input_height_padded = (C * K * K + 15)/16 * 16;
    int input_width_padded = (H_out * W_out + 15)/16 * 16;
    //Padded output
    int output_height_padded = mask_height_padded;
    int output_width_padded = input_width_padded;

    int num_input_rows = input_height_padded;       //C * K * K;
    int num_input_columns = input_width_padded;     //H_out * W_out;
    int num_mask_rows = mask_height_padded;         //M;
    int num_mask_columns = mask_width_padded;       //C * K * K;
    int num_output_rows = output_height_padded;     //M;
    int num_output_columns = output_width_padded;   //H_out * W_out;
    ...
    #define uin_3d(i2, i1, i0) input[(i2) * (input_height_padded * input_width_padded) + (i1) * (input_height_padded) + (i0)]
...
__global__ void unroll_Kernel(const half* device_input, half* device_input_unrolled, int width_padded, int height_padded, int C_padded, const int B, const int M, const int C, const int H, const int W, const int K, const int S) {
    ...
    #define uin_3d(i2, i1, i0) device_input_unrolled[(i2) * (height_padded * width_padded) + (i1) * (width_padded) + (i0)]
    //in_4d(b, c, h, w)
    //uin_3d(b, h_unrolled, w_unrolled)

    if (t < C_padded * width_padded) {
        // Channel of the input feature map being collected by the thread
        int c = t / width_padded;
        ...
        int w_unroll = t % width_padded;
        // Horizontal and vertical indices of the output element
        if ((c < C) && (w_unroll < W_unroll)){    //original useful info
            ...
            for(int h_unroll = c*K*K; (h_unroll < height_padded) && (h_unroll < (c+1)*K*K); h_unroll++){
                uin_3d(b, h_unroll, w_unroll) = __float2half(0.0f);
            }
        }
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    //When converting the mask, we should also do a padding, to make the mask width and height both a multiple of 16.
    int mask_height_padded = ceil(M / 16.0) * 16;
    int mask_width_padded = ceil(C * K * K / 16.0) * 16;
    //When unrolling the input, we should also do a padding, to make the input width and height both a multiple of 16.
    int input_height_padded = ceil(C * K * K / 16.0) * 16;
    int input_width_padded = ceil(H_out * W_out / 16.0) * 16;
    //Padded output
    int output_height_padded = mask_height_padded;
    int output_width_padded = input_width_padded;
    ...
    cudaMalloc((void **)&device_output_fp16, B * output_height_padded * output_width_padded * sizeof(half));
    cudaMalloc((void **)&device_input_fp16, B * C * H * W * sizeof(half));
    cudaMalloc((void **)&device_mask_fp16, mask_height_padded * mask_width_padded * sizeof(half));
    cudaMemset(device_output_fp16, 0, output_height_padded * output_width_padded * sizeof(half));
    cudaMemset(device_input_fp16, 0, B * C * H * W * sizeof(half));
    cudaMemset(device_mask_fp16, 0, mask_height_padded * mask_width_padded * sizeof(half));

    convertFP32toFP16<<<(B * C * H * W + 255) / 256, 256>>>(device_input_fp16, device_input, B * C * H * W);
    cudaDeviceSynchronize();
    dim3 dimGrid_padmask(ceil((mask_height_padded * mask_width_padded * 1.0)/256.0), 1, 1);
    dim3 dimBlock_padmask(256, 1, 1);
    convertFP32toFP16_padded<<<dimGrid_padmask, dimBlock_padmask>>>(device_mask_fp16, device_mask, mask_width_padded, mask_height_padded, M, C, K);
    cudaDeviceSynchronize();
    cudaMemcpyToSymbol(Mask_c, device_mask_fp16, mask_height_padded * mask_width_padded * sizeof(half));
    //---Debug---
    half *host_input_fp16 = (half *)malloc(B * C * H * W * sizeof(half));
    cudaMemcpy(host_input_fp16, device_input_fp16, B * C * H * W * sizeof(half), cudaMemcpyDeviceToHost);
    half *host_mask_fp16 = (half *)malloc(mask_height_padded * mask_width_padded * sizeof(half));
    cudaMemcpy(host_mask_fp16, device_mask_fp16, mask_height_padded * mask_width_padded * sizeof(half), cudaMemcpyDeviceToHost);
    
    std::ofstream outfile;
    outfile.open("host_input_fp16.txt");
    for (int b=0; b<B; b++){
        outfile << "b = " << b << ":-----------------------------------------" << std::endl;
        for (int c=0; c<C; c++){
        outfile << "-- c= " << c << std::endl;
            for (int i = 0; i < H; i++){
                for (int j = 0; j < W; j++){
                    outfile << __half2float(host_input_fp16[b*(C*H*W) + c*(H*W) + i * W + j]) << " ";
                }
                outfile << std::endl;
            }
        }
	...
	outfile.open("host_mask_fp16_padded.txt");
    outfile << "M=" << M << ", C=" << C << ", K=" << K << std::endl;
    for (int i = 0; i < mask_height_padded; i++){
        for (int j = 0; j < mask_width_padded; j++){
            outfile << __half2float(host_mask_fp16[i * mask_width_padded + j]) << " ";
        }
        outfile << std::endl;
    }
    outfile.close();

    free(host_input_fp16);
    free(host_mask_fp16);
    //---Debug---
    // The kernel to unroll the input feature maps-----------------------------------------------------
    half *device_input_unrolled_fp16;
    cudaMalloc((void **)&device_input_unrolled_fp16, B * input_height_padded * input_width_padded * sizeof(half));    
    ...
    int C_padded = ceil(input_height_padded / (1.0*K*K));
    int num_threads = C_padded * input_width_padded;
    ...
    unroll_Kernel<<<dimGrid_unroll, dimBlock_unroll>>>(device_input_fp16, device_input_unrolled_fp16, input_width_padded, input_height_padded, C_padded, B, M, C, H, W, K, S);
    cudaDeviceSynchronize();
    //Kernel ends--------------------------------------------------------------------------------------------------

    //---Debug---
    half *host_input_unrolled_fp16 = (half *)malloc(B * input_height_padded * input_width_padded * sizeof(half));
    cudaMemcpy(host_input_unrolled_fp16, device_input_unrolled_fp16, B * input_height_padded * input_width_padded * sizeof(half), cudaMemcpyDeviceToHost);
    // std::ofstream outfile;
	outfile.open("host_input_unrolled_fp16.txt");
    for (int b=0; b<B; b++){
        outfile << "b = " << b << ":-----------------------------------------" << std::endl;
        for (int i = 0; i < input_height_padded; i++){
            for (int j = 0; j < input_width_padded; j++){
                outfile << __half2float(host_input_unrolled_fp16[b*(input_height_padded*input_width_padded) + i * input_width_padded + j]) << " ";
            }
            outfile << std::endl;
        }
    }
    ...
    const int X_grid = ceil((1.0 * output_width_padded) / (1.0 * TILE_WIDTH));
    const int M_grid = ceil((1.0 * output_height_padded) / (1.0 * TILE_WIDTH));    //Number of tiles in height for all output feature maps in an image
    ...
    std::cout<<"mask_height_padded: "<<mask_height_padded<<std::endl;
    std::cout<<"mask_width_padded: "<<mask_width_padded<<std::endl;
    std::cout<<"input_height_padded: "<<input_height_padded<<std::endl;
    std::cout<<"input_width_padded: "<<input_width_padded<<std::endl;
    std::cout<<"output_height_padded: "<<output_height_padded<<std::endl;
    std::cout<<"output_width_padded: "<<output_width_padded<<std::endl;
    std::cout<<"C_padded: "<<C_padded<<std::endl;
    ...
    //---Debug---
    half *host_output_fp16 = (half *)malloc(B * output_height_padded * output_width_padded * sizeof(half));
    cudaMemcpy(host_output_fp16, device_output_fp16, B * output_height_padded * output_width_padded * sizeof(half), cudaMemcpyDeviceToHost);
    // std::ofstream outfile;
	outfile.open("host_output_fp16.txt");
    for (int b=0; b<B; b++){
        outfile << "b = " << b << ":-----------------------------------------" << std::endl;
        for (int i = 0; i < output_height_padded; i++){
            for (int j = 0; j < output_width_padded; j++){
                outfile << __half2float(host_output_fp16[b*(output_height_padded*output_width_padded) + i * output_width_padded + j]) << " ";
            }
            outfile << std::endl;
        }
    }
    ...
    convertFP16toFP32_unpadded<<<(B * output_width_padded * output_height_padded + 255) / 256, 256>>>(device_output, device_output_fp16, output_width_padded, output_height_padded, B, M, H_out, W_out);
    cudaDeviceSynchronize();
...
__host__ void get_device_properties_to_file()
{
    int deviceCount;
    cudaGetDeviceCount(&deviceCount);
    std::ofstream outfile;
    outfile.open("device_properties.txt");
    for(int dev = 0; dev < deviceCount; dev++)
    {
        cudaDeviceProp deviceProp;
        cudaGetDeviceProperties(&deviceProp, dev);

        outfile<<"Device "<<dev<<" name: "<<deviceProp.name<<std::endl;
        outfile<<"Computational capabilities: "<<deviceProp.major<<"."<<deviceProp.minor<<std::endl;
        outfile<<"Max Global memory size: "<<deviceProp.totalGlobalMem<<std::endl;
        outfile<<"Max Constant memory size: "<<deviceProp.totalConstMem<<std::endl;
        outfile<<"Max Shared memory size per block: "<<deviceProp.sharedMemPerBlock<<std::endl;
        outfile<<"Max threads per block: "<<deviceProp.maxThreadsPerBlock<<std::endl;
        outfile<<"Max block dimensions: "<<deviceProp.maxThreadsDim[0]<<" x, "<<deviceProp.maxThreadsDim[1]<<" y, "<<deviceProp.maxThreadsDim[2]<<" z"<<std::endl;
        outfile<<"Max grid dimensions: "<<deviceProp.maxGridSize[0]<<" x, "<<deviceProp.maxGridSize[1]<<" y, "<<deviceProp.maxGridSize[2]<<" z"<<std::endl;
