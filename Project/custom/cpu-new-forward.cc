...
void conv_forward_cpu(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
  ...
  for (int b=0; b<B; b++){      //Loop for each image in the batch
    for (int m=0; m<M; m++){    //Loop for each output feature maps
      for (int h=0; h<H_out; h++){  //Loop for each output element (H_out*W_out elements)
        for (int w=0; w<W_out; w++){
          out_4d(b, m, h, w) = 0;
          for (int c=0; c<C; c++){    //sum over all input feature maps
            for (int p=0; p<K; p++){  //Loop for the K*K filter
              for (int q=0; q<K; q++){
