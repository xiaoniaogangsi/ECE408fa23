...
#define CUDA_MAX_NUM_THREADS 1024   //The maximum number of threads in a block, we assume it is 1024
...
// __global__ void conv_forward_kernel(float *output, const float *input, const float *mask, float *input_unrolled, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    // #define uin_3d(i2, i1, i0) input_unrolled[(i2) * (C * K * K * H_out * W_out) + (i1) * (H_out * W_out) + (i0)]
...
// __global__ void unroll_Kernel(const float* device_input, float* device_input_unrolled, const int B, const int M, const int C, const int H, const int W, const int K, const int S) {
...
//                 }else{
//                     uin_3d(b, h_unroll, w_unroll) = 0.0f;
//                 }
//             }
//         }
//     }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    cudaMalloc((void **)&device_input_unrolled, B * C * K * K * H_out * W_out * sizeof(float));    
    ...
    // dim3 dimBlock_unroll(CUDA_MAX_NUM_THREADS, 1, 1);
    // unroll_Kernel<<<dimGrid_unroll, dimBlock_unroll>>>(device_input, device_input_unrolled, B, M, C, H, W, K, S);
    ...
    // conv_forward_kernel<<<dimGrid, dimBlock>>>(device_output, device_input_unrolled, device_mask, B, M, C, H, W, K, S);
    // conv_forward_kernel<<<dimGrid, dimBlock>>>(device_output, device_input, device_mask, device_input_unrolled, B, M, C, H, W, K, S);
    cudaDeviceSynchronize();
