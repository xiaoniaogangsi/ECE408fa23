...
#define TILE_WIDTH 16  //TILE_WIDTH == BLOCK_WIDTH always holds
...
__global__ void matrixMultiplyShared(float *A, float *B, float *C,
  ...
  //Note that here (numAColumns-1) / TILE_WIDTH + 1 == ceil(numAColumns/TILE_WIDTH)
  for (int q=0; q < (numAColumns-1) / TILE_WIDTH + 1; q++){  //numAColumns == numBRows
    //load subtiles of A and B from the global memory to the shared memory of this block
    if (Row < numARows && q*TILE_WIDTH+tx < numAColumns){ //Boundary condition
      subTileA[ty][tx] = A[Row * numAColumns + q * TILE_WIDTH + tx];
    }else{
      subTileA[ty][tx] = 0;
    }
    if (q*TILE_WIDTH+ty < numBRows && Col < numBColumns) { //Boundary condition condition
    ...
    //Use the subtiles to calculate the partial sum for the element, and accumulate into Cvalue
    if (Row < numCRows && Col < numCColumns){ //Boundary condition
  ...
  if (Row < numCRows && Col < numCColumns){ //Boundary condition
    C[Row * numCColumns + Col] = Cvalue;  //Store the result element back
  }
}
...
int main(int argc, char **argv) {
  ...
  //Because of row major organization of the matrices, our first dimension should be the column.
