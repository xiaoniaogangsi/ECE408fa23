...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  int row = blockIdx.x * blockDim.x + threadIdx.x;  //One thread takes care of a row, not this row number is the new row number after sort
  if (row < dim) {  //Boundary check
    float dot = 0;  //Used to hold the products of this matrix row and the vector
    unsigned int sec = 0;   //Enumerate each section (section means the columns of JDS / the rows of JDS_T)
    while (sec < matRows[row]) {   //"matColStart[sec+1] - matColStart[sec] > row" may also work (Unfortunately not, maybe sec+1 leads to segmentation fault)
      //If the element in this row and this section is still a non-zero element, we need to do the multiplication for this element
      //Else, it means that all non-zero elements in this row is done, the rest sections can be ignored, and the job of this thread is done
      unsigned int data_idx = matColStart[sec] + row;   //Get the index of the element in the matData array (also in the matCols array)
      unsigned int col = matCols[data_idx];   //Get the column index in the sparse matrix, which can be used to reference the corresponding row of the vector
      dot += matData[data_idx] * vec[col];    //Accumulate the dot product
      sec++;  //Increment the section index
    }
    unsigned int true_row = matRowPerm[row];  //Get the true row index in the sparse matrix according to matRowPerm array
    out[true_row] = dot;   //Write the result to the output array
  }
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  dim3 dimBlock(BLOCK_SIZE, 1, 1);    //Each block takes care of BLOCK_SIZE rows using BLOCK_SIZE threads
